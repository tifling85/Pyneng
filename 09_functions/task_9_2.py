...
def generate_trunk_config(intf_vlan_mapping, trunk_mode_template):
    ...
    for intf, vlans in intf_vlan_mapping.items():
        lst_res.append(('interface {}'.format(intf)).rstrip())
        for command in trunk_mode_template:
            if command.find('allowed vlan') != -1:
                lst_res.append((command + ' ' + ','.join(str(vlan) for vlan in vlans)).rstrip())
            lst_res.append(command)
    return lst_res

lst_res = generate_trunk_config(trunk_config, trunk_mode_template)
print(lst_res)
