...
def convert_config_to_dict(cfg_file):
    dict_comm = {}
    with open(cfg_file) as f:
        for line in f:
            line = line.rstrip()
            if line.startswith('!') or ignore_command(line, ignore) or not line: continue
            if not line.startswith(' '):
                cur_key = line
                dict_comm[cur_key] = []
            else:
                dict_comm[cur_key].append(line.lstrip())
    return dict_comm


dict_comm = convert_config_to_dict('config_sw1.txt')
for i in dict_comm:
    print(i, dict_comm[i])
