...
def convert_ranges_to_ip_list(lst_ranges_ip):
    lst_ip = []
    for rang in lst_ranges_ip:
        try:
            ipaddress.ip_address(rang)
            lst_ip.append(rang)
            continue
        except ValueError:
            pass
        start_ip = ipaddress.ip_address(rang.split('-')[0])
        if rang.split('-')[-1].isdigit():
            for diff_oct in range(int(rang.split('-')[-1]) - int(start_ip.compressed.split('.')[-1]) + 1):
                lst_ip.append((start_ip + diff_oct).compressed)
        else:
            end_ip = ipaddress.ip_address(rang.split('-')[-1])
            for diff_oct in range(int(end_ip.compressed.split('.')[-1]) - int(start_ip.compressed.split('.')[-1]) + 1):
                lst_ip.append((start_ip + diff_oct).compressed)
