...
def generate_description_from_cdp(conf_file):
    res_dict = {}
    template = 'description Connected to {} port {}'
    with open(conf_file) as f:
        regex = r'^(?P<device>\S+) +(?P<lintf>Eth \S+).+(?P<port>Eth \S+)$'
        for line in f:
            match = re.search(regex, line)
            if not match: continue
            res_dict[match.group('lintf')] = template.format(match.group('device'), match.group('port'))
    return res_dict
