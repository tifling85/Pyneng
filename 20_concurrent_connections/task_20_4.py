...
def send_config(device, command):
    with netmiko.ConnectHandler(**device) as ssh:
        logging.info(f'Cheking {device["ip"]}')
        ssh.enable()
...
def send_commands_to_devices(devices, show=None, config=None, filename='for_task_20_4.txt', limit=3):
    with ThreadPoolExecutor(max_workers = limit) as executor:
        with open(filename, 'w') as f:
            if show:
                res_iter = executor.map(send_command, devices, itertools.repeat(show))
                for res in res_iter:
                    f.write(res)
            if config:
                res_iter = executor.map(send_config, devices, itertools.repeat(config))
                for res in res_iter:
                    f.write(res)
