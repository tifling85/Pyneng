...
def create_vpn_config(template1, template2, data):
    env = Environment(loader=FileSystemLoader(os.path.dirname(template1)), trim_blocks=True, lstrip_blocks=True)
    temp1 = env.get_template(os.path.basename(template1))
    temp2 = env.get_template(os.path.basename(template2))
    return (temp1.render(data), temp2.render(data)) 

if __name__ == '__main__':
    template1 = os.path.abspath("templates/gre_ipsec_vpn_1.txt")
    template2 = os.path.abspath("templates/gre_ipsec_vpn_2.txt")
    str_conf = create_vpn_config(template1, template2, data)
    print(str_conf[0])
    print(str_conf[1])
